...
public class LE {
    ...
    /**
     * @param grid_res the velocity grid resolution
     * @param N basis dimension (eigenfunction basis?)
     * @param density_grid_res density grid resolution (TODO: what is this?)
     */
    LE(int grid_res, int N,int density_grid_res) {
        ...
        // Cheng: is the velocity of a particle interpolated from close
        // velocity vectors?
        ...
        // Cheng: this lookup table links the ith dimension with some kx and ky by
        // i = kx * N_sqrt + ky
        this.fill_lookup_table();

        System.out.println("Precomputing Basis Fields.");
        // Cheng: I guess this is the C_k matrix mentioned on page 6 of the paper
        // TODO: need to check how this could be expanded to 3D
        this.precompute_basis_fields();

        // Cheng: this dynamics computation is the most confusing
    ...
    public double[][][] basis_field_2d_rect(int n,int m,double amp) {
		...
		// Cheng: mx and my are dimensions of velocity field
		...
		for (int i = 0;i<this.mx+1;i++) {
			for (int j=0;j<this.my+1;j++) {
				// Cheng: x and y are incrementing from 0 to pi
